...
class Spaceship:
    ...
    def update_rotation(self):
        ...
        self.mask = pygame.mask.from_surface(self.rotatedSurf) # Create a mask from the rotated surface for pixel-perfect collision detection
    ...
    def collision_detection(self, obj):
        if isinstance(obj, Asteroids):
            # Calculate the offset between the asteroid and the spaceship's rotated rectangle
            offset_x = obj.x - self.rotatedRect.left
            offset_y = obj.y - self.rotatedRect.top

            # Check if the masks overlap at the calculated offset
            if self.mask.overlap(obj.mask, (offset_x, offset_y)):
                if obj not in self.colliding_asteroids: # If there's a collision and the asteroid is not already colliding with the spaceship
                    damage_sound.play()
                    self.life_deduction()
                    self.colliding_asteroids.add(obj) # Add this asteroid to the set of colliding asteroids
            else:
                # If there's no collision but the asteroid was previously colliding
                if obj in self.colliding_asteroids:
...
class Asteroids:
    def __init__(self, img, size):
        ...
        self.mask = pygame.mask.from_surface(pygame.transform.scale(self.img, self.size))
